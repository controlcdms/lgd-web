      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <div className="text-white/60 mt-1">{t("licenses.subtitle")}</div>
          </div>
        </div>

        <div className="rounded-2xl border border-white/10 bg-white/5 p-4 mb-6">
          <div className="text-sm font-medium">{t("licenses.statusTitle")}</div>
          <div className="text-sm text-white/60 mt-1">{t("licenses.statusHint")}</div>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-6">
          {(Object.entries(LICENSE_META) as Array<
            [LicenseType, (typeof LICENSE_META)[LicenseType]]
          >).map(([type, meta]) => (
            <div
              key={type}
            ...
            >
              ...
              <div className="mt-4 flex items-center gap-2">
                ...
                >
                  {busy && selected === type ? t("common.creating") : t("licenses.buy1m")}
                </button>
          ...
          >
            ...
            >
              <div className="text-lg font-semibold">
                {selectedMeta?.title || t("licenses.title")}
              </div>
