...
async function resolveProductId(uid: number, apiKey: string, type: LicenseType): Promise<number> {
...
export async function POST(req: Request) {
  try {
    ...
    const meRows = await odooSearchReadAsUser(rpcAuth.uid, rpcAuth.apiKey, "res.users", [["id", "=", odooUserId]], ["id", "login", "partner_id"], 1);
    const me = meRows?.[0];
    ...
    const productId = await resolveProductId(rpcAuth.uid, rpcAuth.apiKey, type);

    const orderId = await odooCallAsUser<number>(rpcAuth.uid, rpcAuth.apiKey,  "sale.order", "create", [{
      partner_id: partnerId,
      user_id: odooUserId,
      client_order_ref: `LGD-LIC-${type}-${rpcAuth.login}`,
    }]);

    await odooCallAsUser<number>(rpcAuth.uid, rpcAuth.apiKey,  "sale.order.line", "create", [{
      order_id: orderId,
